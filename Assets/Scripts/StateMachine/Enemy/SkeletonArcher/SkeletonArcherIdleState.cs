...
public class SkeletonArcherIdleState : EnemyState
{
    private SkeletonArcherStateMachine sm;

    public SkeletonArcherIdleState(SkeletonArcherStateMachine skeletonArcherStateMachine) : base(skeletonArcherStateMachine)
    {
        sm = skeletonArcherStateMachine;
    }
        ...
        if (GetCurrentTarget() != null)
        {
            ...
            if (sm.isAggro)
            {
                ...
                if (playingAnimation)
                {
                    ...
                    {
                        sm.SwitchState(new SkeletonArcherChasingState(sm));
                    }
                }
                else sm.SwitchState(new SkeletonArcherChasingState(sm));
                return;
            }
            else
            {
